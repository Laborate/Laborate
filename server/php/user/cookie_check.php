...
function cookieCheck() {
    ...
    if(isset($_COOKIE['userLogin'])) {
        $query_Login = sprintf("SELECT * FROM users, login WHERE login.login_user_id = users.user_id AND login.login_uuid = ".GetSQLValueString($_COOKIE['userLogin'], "text"));
        $Login = mysql_query($query_Login , $database) or die(mysql_error());
        $row_Login = mysql_fetch_assoc($Login);

        if($row_Login['login_uuid'] == $_COOKIE['userLogin']) {
                ...
                $deleteSQL = sprintf("DELETE FROM login WHERE login.login_uuid = '".$_COOKIE['userLogin']."'");
