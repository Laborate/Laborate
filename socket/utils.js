...
module.exports = {
    session_authentication: function(data, callback) {
        ...
        user_mysql.query(session_pull, function(err, sql_results) {
        ...
        user_mysql.query(session_pull, function(err, sql_results) {
            if(!err && sql_results.length != 0) {
                ...
                */
                ...
                session_push += user_mysql.escape(json_final_document) + " WHERE session_id = " + user_mysql.escape(session);
                user_mysql.query(session_push);
            } else {
                callback(false);
            }
